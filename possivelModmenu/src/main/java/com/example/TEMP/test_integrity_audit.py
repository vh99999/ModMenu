...
class TestIntegrityAudit(unittest.TestCase):
    ...
    def test_active_policy_immutability(self):
        ...
        active_policy = MLPolicy(self.server.model_manager, mode="ACTIVE")
        
        # Mutation attempts on ACTIVE policy should be REJECTED (but not crash the process)
        loss = active_policy.update_weights([{"experience_id": "test", "reward": 1.0}])
    ...
    def test_shadow_policy_learning_authorized(self):
        ...
        """
        from policy import MLPolicy
        shadow_policy = MLPolicy(self.server.model_manager, mode="SHADOW")
