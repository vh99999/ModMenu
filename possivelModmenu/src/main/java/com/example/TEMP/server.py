...
class AIServer:
    ...
    def __init__(self, host: str = '127.0.0.1', port: int = 5001):
        ...
        self.active_policy = MLPolicy(self.model_manager, mode="ACTIVE")
        self.shadow_policy = MLPolicy(self.model_manager, mode="SHADOW")
        ...
        try:
            with conn:
                ...
                received_lineage = {
                    ...
                    "learning_allowed": True, # Canonical check moved to LearningGateSystem
                ...
                # Learning gates and Trainer will enforce specific freeze rules
                if intent_taken:
                    if self.auditor.check_quality_gate(audit_entry):
                        ...
                        try:
                            ...
                            experience_for_trainer = {
                                ...
                                "policy_mode": self.shadow_policy.mode
                            }
                            ...
                            logger.info(f"FINAL_LEARNING_DECISION: {self.learning_gate_system.get_final_decision(gate_decisions)}")
                            ...
                            # 3.4 LIVE_SHADOW_LEARNING allowed for ShadowPolicy ONLY if gates pass
                            # AND system is NOT in SHADOW_LEARNING_PAUSE or LOCKDOWN mode.
                            can_learn = self.learning_gate_system.get_final_decision(gate_decisions) == "SHADOW_LEARNING_ALLOWED"
                            if self.failure_manager.mode in [DisasterMode.SHADOW_LEARNING_PAUSE, DisasterMode.LOCKDOWN]:
                                logger.warning(f"SHADOW_LEARNING_PAUSED: Learning blocked for {experience_id} due to system mode ({self.failure_manager.mode}).")
                                can_learn = False
                                
                            if can_learn:
                                self.trainer.train_on_experience(experience_for_trainer)
                            else:
                                logger.warning(f"[SHADOW] Experience rejected (Gates or Pause) for {experience_id}")
                ...
                if self.control_mode == "HUMAN":
                    ...
                    _, _, _, _, _, _, shadow_data = self.arbitrator.decide(normalized_state)
    ...
    def _run_drift_audit(self):
        ...
        if drift_count == 0:
            ...
            self.failure_manager.set_mode(DisasterMode.SHADOW_LEARNING_PAUSE)
