...
class MitigationType(str, Enum):
    ...
    SHADOW_LEARNING_PAUSE = "SHADOW_LEARNING_PAUSE"
    ACTIVE_KILLSWITCH = "ACTIVE_KILLSWITCH"
...
class Monitor:
    ...
    def _determine_mitigation(self, metrics: Dict[str, Any], alerts: List[Dict[str, Any]]) -> MitigationType:
        # 1. Active Policy Kill-switch Triggers
        if metrics.get("survival_rate", 1.0) < 0.3:
            return MitigationType.ACTIVE_KILLSWITCH
        
        # 2. Shadow Learning Pause Triggers (System-level data/contract issues)
        if metrics.get("violation_rate", 0.0) > 0.15:
            return MitigationType.SHADOW_LEARNING_PAUSE

        # 3. Soft Degrade Triggers (Performance issues)
        if metrics.get("ml_failure_rate", 0.0) > 0.05 or metrics.get("ml_timeout_rate", 0.0) > 0.02:
