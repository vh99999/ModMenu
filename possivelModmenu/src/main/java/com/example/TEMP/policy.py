...
class MLPolicy(LearnedPolicy):
    ...
    """
    def __init__(self, model_manager: 'ModelManager', mode: str = "ACTIVE"):
        self.manager = model_manager
        self.mode = mode # ACTIVE or SHADOW
    ...
    def decide(self, state: Dict[str, Any]) -> Tuple[str, float, Optional[str], Dict[str, float], str]:
        """Wraps decide_version based on policy mode."""
        ...
        if self.mode == "SHADOW" and self.manager.candidate_model_version:
            version = self.manager.candidate_model_version
            
        return self.decide_version(state, version)
    ...
    def update_weights(self, batch: List[Dict[str, Any]]) -> float:
        ...
        Enforces immutability for ACTIVE mode and learning for SHADOW mode.
        """
        if self.mode == "ACTIVE":
            # Requirement 1 & 4 (Phase 10B): ActivePolicy MUST be immutable. 
            logger.error("[ACTIVE] REJECT: Mutation attempt on immutable ACTIVE policy! Shadow learning ONLY.")
            return 0.0
        ...
        logger.info(f"[SHADOW] Experience accepted: ShadowPolicy updated. Total Boost: {self.learned_boost:.4f}")
...
class PolicyArbitrator:
    ...
    def decide(self, state: Dict[str, Any]) -> Tuple[str, float, str, str, Optional[str], Optional[str], Optional[Dict[str, Any]]]:
        ...
        1. SHADOW EVALUATION (Always run - ABSOLUTE LAW)
        2. HEURISTIC VETO (Survival Guardian)
        3. ML SUGGESTION (Learned path, if enabled)
        4. HEURISTIC FALLBACK (Safe path)
        5. RANDOM FALLBACK (Entropy path)
        ...
        # 1. Shadow Evaluation (ABSOLUTE LAW: SHADOW MODE MUST NEVER BE DISABLED)
        # 1.1 Candidate Model Evaluation
        if candidate_ver:
        ...
        # 1.2 Dual-Policy Shadow Evaluation
        # We always query shadow policy to maintain its performance metrics
        s_intent, s_conf, s_ver, s_probs, s_auth = self.shadow_policy.decide(state)
        # If candidate_ver evaluation didn't run or we want to include shadow_policy data:
        # Re-defining shadow_data to include shadow_policy results as primary shadow signal
        ...
        if h_conf >= 0.9:
            # We still run ML in shadow mode if it didn't run already for divergence tracking
            # But here we focus on the return path for ACTIVE control
            return h_intent, h_conf, "HEURISTIC_VETO", h_auth, "SAFETY_RULE_TRIGGERED", h_ver, shadow_data

        # 3. Active ML Suggestion Path
        # Soft Degrade Check: If in cooldown, bypass ACTIVE ML
        ...
        if self.ml_enabled:
            # Decide if we use Canary (Candidate) or Production for ACTIVE control
            use_candidate = False
            ...
            ml_intent, ml_conf, ml_ver, ml_probs, ml_auth = self.active_policy.decide_version(state, target_version)
            ...
            if self.influence_weight > 0 and random.random() < self.influence_weight:
                # Shadow influence (Mix Action)
                if s_intent and s_intent != Intent.STOP.value:
                    final_intent, final_conf, final_ver, final_auth = s_intent, s_conf, s_ver, s_auth
            ...
            # 3.1 Active ML Timeout Handling
            ...
            # 3.2 Active ML UNCERTAINTY & ENTROPY CHECK
            ...
            # 3.3 Active ML Confidence & Output Validation
        ...
        # 4. Fallback Path (ACTIVE ML DISABLED)
    ...
    def apply_mitigation(self, mitigation: str):
        ...
        elif mitigation == "ACTIVE_KILLSWITCH":
            if self.ml_enabled:
                self.ml_enabled = False
                logger.critical("ARBITRATOR: ACTIVE ML disabled via kill-switch (Monitor Recommendation).")
        elif mitigation == "SHADOW_LEARNING_PAUSE":
            # SHADOW_LEARNING_PAUSE is handled by AIServer and Trainer.
            # Arbitrator does not pause shadow evaluation (ABSOLUTE LAW).
            logger.warning("ARBITRATOR: Received SHADOW_LEARNING_PAUSE. Shadow evaluation continues; learning paused.")
